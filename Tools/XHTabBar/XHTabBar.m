...
//title选中颜色
#define TitleColor_Sel  RGBCOLOR(33,199,185)
//title字体大小
...
static const NSInteger buttonBaseTag = 8975;
static const NSInteger numLabBaseTag = 9975;
...
{
    //控制器数组(不需要导入控制器头文件)
    self.controllerArray = @[@"XHLiveViewController",@"XHPushFlowViewController",@"XHMeViewController"];
    self.titleArray = @[@"首页",@"直播",@"我的"];
    self.imageArray = @[@"tab_live",@"query_tabbar",@"tab_me"];
    self.selImageArray = @[@"tab_live_p",@"query_tabbar_sel",@"tab_me_p"];
...
-(void)createControllerBycontrollerArrayay:(NSArray *)controllerArray
{
    if(controllerArray.count==0) NSLog(@"控制器数组为nil,请初始化");
...
-(void)buttonAction:(UIButton *)button{
    ...
    NSInteger index = button.tag-buttonBaseTag;
...
-(void)showControllerIndex:(NSInteger)index
{
    ...
    UIButton *button = (UIButton *)[self.tabBarView viewWithTag:buttonBaseTag+index];
    button.selected = YES;
    self.seleBtn = button;
...
{
    UILabel *numLabel = (UILabel *)[self.tabBarView viewWithTag:numLabBaseTag+index];
    numLabel.hidden=NO;
